...
class ShowcaseManager private constructor(private val builder: Builder) {
    ...
    class Builder {
        ...
        fun topView (topView: Int): Builder {
            this.top = topView
            return this
        }
        fun bottomView (bottomView: Int): Builder {
        ...
        private fun createShowcaseModel(): ShowcaseModel {
            val myrect : Rect =  Rect(view.left,view.top,view.right,view.bottom)
            val viewPositionRect = Rect()
            view.getGlobalVisibleRect(viewPositionRect)
            Log.e(TAG, "createShowcaseModel: getGlobalVisibleRect ==> " +  view.getGlobalVisibleRect(viewPositionRect))
            ...
            //val rect = calculateRect(marginFocusArea, view)
            val rect = calculateRect(marginFocusArea, myrect)
        ...
        private fun calculateRect(marginFocusArea: Int, viewPositionRect: Rect): Rect {
            ...
            /*Log.e(TAG, "calculateRect =>> top ==> " + (viewPositionRect.top))
            Log.e(TAG, "calculateRect =>> top - margin ==> " + (viewPositionRect.top - margin).toInt() )
            Log.e(TAG, "calculateRect =>> bottom ==> " + (viewPositionRect.bottom))
            Log.e(TAG, "calculateRect =>> bottom - margin ==> " + (viewPositionRect.bottom - margin).toInt())*/
            return Rect(
        ...
        private fun calculateRect(marginFocusArea: Int, viewPositionRect: View): Rect {
            val margin = ShowcaseUtils.convertDpToPx(marginFocusArea.toFloat())
            Log.e(TAG, "calculateRect =>> top ==> " + (viewPositionRect.top).toInt() )
            Log.e(TAG, "calculateRect =>> top - margin ==> " + (viewPositionRect.top - margin).toInt() )
            Log.e(TAG, "calculateRect =>> bottom ==> " + (viewPositionRect.bottom).toInt())
            Log.e(TAG, "calculateRect =>> bottom - margin ==> " + (viewPositionRect.bottom - margin).toInt())
            return Rect(
                    (viewPositionRect.left - margin).toInt(),
                    (viewPositionRect.top - margin).toInt(),
